...
#define IOM_FPGA_PUSH_SWITCH_ADDRESS 0x07000017			// pysical address
...
ssize_t iom_fpga_push_switch_read(struct file *inode, char *gdata, size_t length, loff_t *off_what) 
{
	...
	for(i=0;i<length;i++) 
		push_sw_value[i] = inb((unsigned int)iom_fpga_push_switch_addr+i);
...
int __init iom_fpga_push_switch_init(void)
{
	...
	iom_fpga_push_switch_addr = ioremap(IOM_FPGA_PUSH_SWITCH_ADDRESS, 0x9);
